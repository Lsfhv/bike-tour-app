...
class JourneyData {
  ...
  _routeOptimize() async {
    ...
    List<LatLng> list = List<LatLng>.generate(
        _destinations.length, (i) => _destinations[i].position);
    await DirectionsRepository()
        .getDirections(
            origin: _currentPosition.center as LatLng,
            ending_bike_dock: _currentPosition.center as LatLng,
            destinations: list,
            optimize: true)
        .then((value) => {_rerouteWaypoints(value)});
    //get directions
    //
  }
  ...
  _init_() async {
    ...
    markers
        .add(UserMarker(user: UserPosition(_currentPosition.center as LatLng)));
    ...
    for (int i = -1; i < _destinations.length - 1; i++) {
      ...
      if (i == -1) {
        ...
        starting_dock = _choosingDock(
            bike_api.getNearbyDocks((start as UserPosition).center as LatLng));
      } else {
        start = _destinations[i];
        end = _destinations[i + 1];
        starting_dock = _choosingDock(
            bike_api.getNearbyDocks((start as Destination).position));
      }
      /*
...
class _RoutingMap extends State<RoutingMap> {
  ...
  void _generateRoute(JourneyData args) async {
    ...
    final directions = await DirectionsRepository().getDirections(
        origin: origin,
        ending_bike_dock: destination,
        destinations: args.waypoints);
    setState(() {
      _info = directions;
